#!/bin/bash
# Script to deploy NPM xRegistry to Azure Container Instances
...
DNS_NAME_LABEL="xregistry-npm-bridge"
PORT=3100
CPU_CORES=1.0
MEMORY_GB=1.5
API_KEY=""
...
function show_help {
    ...
    echo "  --container-name <n>       Container name (default: xregistry-npm-bridge)"
    echo "  --dns-label <label>        DNS name label (default: xregistry-npm-bridge)"
    echo "  --port <port>              Container port (default: 3100)"
...
IMAGE_NAME="xregistry-npm-bridge"
...
ENV_VARS="NODE_ENV=production PORT=${PORT} XREGISTRY_NPM_PORT=${PORT} XREGISTRY_NPM_BASEURL=${BASEURL} XREGISTRY_NPM_QUIET=false"

# Add API key if provided
if [ -n "${API_KEY}" ]; then
    ENV_VARS="${ENV_VARS} XREGISTRY_NPM_API_KEY=${API_KEY}"
fi
