#!/bin/bash
# Script to deploy Maven Central xRegistry to Azure Container Apps
...
LOCATION="westeurope"
APP_NAME="xregistry-maven-bridge"
...
function show_help {
    ...
    echo "  --app-name <name>          Container App name (default: xregistry-maven-bridge)"
    echo "  --port <port>              Container port (default: 3300)"
...
ENV_VARS="NODE_ENV=production PORT=$PORT XREGISTRY_MAVEN_PORT=$PORT XREGISTRY_MAVEN_BASEURL=$BASEURL XREGISTRY_MAVEN_QUIET=false"

# Add API key if provided
if [ -n "$API_KEY" ]; then
    ENV_VARS="$ENV_VARS XREGISTRY_MAVEN_API_KEY=$API_KEY"
...
# Prepare image name
IMAGE_NAME="ghcr.io/$GITHUB_USER/xregistry-package-registries/xregistry-maven-bridge:$IMAGE_TAG"
