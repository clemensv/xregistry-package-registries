...
# Set up logging
LOG_FILE="/app/logs/nuget-server.log"
CRASH_LOG_FILE="/app/logs/nuget-crashes.log"
mkdir -p /app/logs
...
log_with_timestamp "NuGet server wrapper started"
...
while true; do
    ...
    if [ \$EXIT_CODE -eq 0 ]; then
        log_with_timestamp "NuGet server exited normally"
        break
    else
        log_with_timestamp "NuGet server crashed with exit code \$EXIT_CODE"
        log_crash \$EXIT_CODE
        
        # Wait before restarting
        sleep 5
        log_with_timestamp "Restarting NuGet server..."
    fi
done
EOF

# Make the script executable and fix line endings
RUN chmod +x /app/nuget/restart-wrapper.sh && \
    sed -i 's/\r$//' /app/nuget/restart-wrapper.sh
