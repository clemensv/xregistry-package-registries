...
describe('Angular Packages Integration Test', function() {
  ...
  before(async function() {
    ...
    try {
      ...
      console.log(`Testing with ${angularPackages.length} packages:`, angularPackages.map(p => p.name));
  ...
  describe('Angular Package Discovery', function() {
    it('should find Angular packages in the package list', async function() {
      ...
      const response = await axios.get(`${baseUrl}/noderegistries/npmjs.org/packages?filter=@angular&limit=20`);
      expect(response.status).to.equal(200);
      ...
      console.log(`Found ${packageNames.length} Angular packages through filtering`);
    });
  });
    ...
    angularPackages.forEach((angularPackage, index) => {
      describe(`Package: ${angularPackage.name}`, function() {
        ...
        it('should retrieve package details', async function() {
          ...
          try {
            const response = await axios.get(`${baseUrl}/noderegistries/npmjs.org/packages/${encodedPackageName}`);
            expect(response.status).to.equal(200);
            ...
            expect(packageData).to.have.property('packageid', normalizedPackageId);
        ...
        it('should retrieve package versions', async function() {
          ...
          try {
            const response = await axios.get(`${baseUrl}/noderegistries/npmjs.org/packages/${encodedPackageName}/versions`);
            expect(response.status).to.equal(200);
            ...
            console.log(`✓ Found ${versionIds.length} versions for ${packageName}`);
            
            // Test retrieving a specific version
            const firstVersionId = versionIds[0];
            const versionResponse = await axios.get(
              `${baseUrl}/noderegistries/npmjs.org/packages/${encodedPackageName}/versions/${encodeURIComponent(firstVersionId)}`
            );
            ...
            console.log(`✓ Retrieved version ${firstVersionId} for ${packageName}`);
        ...
        it('should retrieve package documentation', async function() {
          ...
          try {
            const response = await axios.get(`${baseUrl}/noderegistries/npmjs.org/packages/${encodedPackageName}/doc`);
            expect(response.status).to.equal(200);
        ...
        it('should retrieve package metadata', async function() {
          ...
          try {
            const response = await axios.get(`${baseUrl}/noderegistries/npmjs.org/packages/${encodedPackageName}/meta`);
            expect(response.status).to.equal(200);
  ...
  describe('Edge Cases and Error Handling', function() {
    it('should handle non-existent package gracefully', async function() {
      try {
        await axios.get(`${baseUrl}/noderegistries/npmjs.org/packages/this-package-definitely-does-not-exist-12345`);
    ...
    it('should handle invalid group ID', async function() {
      try {
        await axios.get(`${baseUrl}/noderegistries/invalid-group/packages/@angular/core`);
        expect.fail('Should have thrown an error');
      } catch (error) {
        expect(error.response.status).to.equal(404);
      }
    ...
    it('should handle malformed package names', async function() {
      ...
      for (const name of malformedNames) {
        try {
          await axios.get(`${baseUrl}/noderegistries/npmjs.org/packages/${encodeURIComponent(name)}`);
  ...
  async function getAngularPackagesFromNpm() {
      ...
      npmProcess.on('close', (code) => {
        if (code === 0) {
          try {
            ...
            const angularPackages = packages.filter(pkg => 
              pkg.name.startsWith('@angular/') && 
              pkg.publisher && 
              (pkg.publisher.username === 'angular' || pkg.publisher.username === 'google' || pkg.publisher.username === 'google-wombot')
            );
            resolve(angularPackages);
          } catch (parseError) {
            reject(new Error(`Failed to parse npm search output: ${parseError.message}`));
          }
        } else {
    ...
    return new Promise((resolve, reject) => {
      ...
      const process = spawn('node', [serverPath, '--port', serverPort], {  // Removed --quiet flag
        stdio: ['pipe', 'pipe', 'pipe'],
  ...
  async function waitForServer(url, timeout = 30000) {
    ...
    console.log(`Waiting for server to become ready at ${url} (timeout: ${timeout}ms)...`);
