...
describe("NuGet Filter, Sort, and Inline Functionality Integration Tests", function () {
  ...
  before(async function () {
    ...
    containerName = `nuget-filter-test-${Date.now()}-${Math.floor(
      Math.random() * 1000
    )}`;
  ...
  describe("Package Filtering Tests", () => {
    it("should support simple text filtering for Microsoft packages", async () => {
      ...
      console.log(
        `Found ${packages.length} Microsoft packages through simple filtering`
      );
    });

    it("should handle legacy simple text filters (backward compatibility)", async () => {
      // Test that simple text filters are normalized to proper xRegistry syntax
      const legacyFilter = "Microsoft"; // Should be converted to name=*Microsoft*
      const properFilter = "name=*Microsoft*"; // Proper xRegistry syntax

      const legacyResponse = await loggedAxiosGet(
        `${baseUrl}/dotnetregistries/nuget.org/packages?filter=${encodeURIComponent(
          legacyFilter
        )}&limit=5`
      );

      const properResponse = await loggedAxiosGet(
        `${baseUrl}/dotnetregistries/nuget.org/packages?filter=${encodeURIComponent(
          properFilter
        )}&limit=5`
      );
      ...
      expect(properResponse.status).to.equal(200);

      // Both should return similar results
      expect(legacyResponse.data).to.be.an("object");
      expect(properResponse.data).to.be.an("object");

      // Results should contain packages with "Microsoft" in the name
      const legacyPackages = Object.keys(legacyResponse.data);
      const properPackages = Object.keys(properResponse.data);

      legacyPackages.forEach((name) => {
        expect(name.toLowerCase()).to.include("microsoft");
      });
      properPackages.forEach((name) => {
        expect(name.toLowerCase()).to.include("microsoft");
      });

      console.log(
        `Legacy filter normalization works: Legacy=${legacyPackages.length} packages, Proper=${properPackages.length} packages`
      );
    });

    it("should normalize various legacy filter formats", async () => {
      const testCases = [
        { legacy: "System", expected: "name=*System*" },
        { legacy: "EntityFramework", expected: "name=*EntityFramework*" },
        { legacy: "Json", expected: "name=*Json*" },
      ];
      ...
      for (const testCase of testCases) {
        ...
        // If results exist, verify they match the expected pattern
        const packages = Object.keys(response.data);
        packages.forEach((name) => {
          expect(name.toLowerCase()).to.include(testCase.legacy.toLowerCase());
        });

        console.log(
          `Legacy filter '${testCase.legacy}' normalized correctly: ${packages.length} packages found`
        );
      }
    });

    it("should reject filters without name constraint", async () => {
      ...
      for (const filter of invalidFilters) {
        ...
        console.log(
          `Filter without name constraint '${filter}' correctly rejected: ${packages.length} packages`
        );
      }
    });

    it("should support xRegistry filter operators", async () => {
      const operators = [
        { filter: "name=*Microsoft*", description: "Wildcard contains" },
        {
          filter: "name!=*TestPackage*",
          description: "Not equals with wildcard",
        },
        { filter: "name=Newtonsoft.Json", description: "Exact match" },
      ];
      ...
      for (const testCase of operators) {
        ...
        const packages = Object.keys(response.data);
        console.log(
          `xRegistry operator test (${testCase.description}): ${packages.length} packages found`
        );
      }
    });
    ...
    it("should support simple text filtering for Json packages", async () => {
      const response = await loggedAxiosGet(
        `${baseUrl}/dotnetregistries/nuget.org/packages?filter=name=*Json*&limit=5`
      );
      ...
      console.log(
        `Found ${packages.length} Json packages through simple filtering`
      );
    });

    it("should support structured filtering with name constraint", async () => {
      const response = await loggedAxiosGet(
        `${baseUrl}/dotnetregistries/nuget.org/packages?filter=name=Newtonsoft.Json&limit=5`
      );
      ...
      console.log(
        `Found ${packages.length} packages matching structured filter 'name=Newtonsoft.Json'`
      );
    });
    ...
    it("should handle empty filter results gracefully", async () => {
      ...
      console.log(
        `Filter for non-existent package returned ${packages.length} results as expected`
      );
    });
    ...
    it("should respect limit parameter in filtered results", async () => {
      const limit = 3;
      const response = await loggedAxiosGet(
        `${baseUrl}/dotnetregistries/nuget.org/packages?filter=name=*System*&limit=${limit}`
      );
      ...
      console.log(
        `Filtered results correctly limited to ${packages.length} packages (limit: ${limit})`
      );
    });
    ...
    it("should support offset parameter in filtered results", async () => {
      ...
      // Get first page
      const firstPageResponse = await loggedAxiosGet(
        `${baseUrl}/dotnetregistries/nuget.org/packages?filter=name=*Microsoft*&limit=${limit}&offset=0`
      );
      ...
      const secondPageResponse = await loggedAxiosGet(
        `${baseUrl}/dotnetregistries/nuget.org/packages?filter=name=*Microsoft*&limit=${limit}&offset=${offset}`
      );
      ...
      if (firstPagePackages.length > 0 && secondPagePackages.length > 0) {
        const hasOverlap = firstPagePackages.some((pkg) =>
          secondPagePackages.includes(pkg)
        );
      ...
      console.log(
        `Offset filtering works: First page ${firstPagePackages.length} packages, second page ${secondPagePackages.length} packages`
      );
    });
  });
  ...
  describe("Package Sorting Tests", () => {
    it("should sort packages alphabetically by default (ascending)", async () => {
      ...
      console.log(
        `Packages are sorted alphabetically by default: ${packages
          .slice(0, 3)
          .join(", ")}...`
      );
    });
    ...
    it("should sort packages by name in descending order", async () => {
      const response = await loggedAxiosGet(
        `${baseUrl}/dotnetregistries/nuget.org/packages?filter=name=*System*&limit=8&sort=name=desc`
      );
      ...
      console.log(
        `Packages sorted in descending order: ${packages
          .slice(0, 3)
          .join(", ")}...`
      );
    });
    ...
    it("should sort packages by packageid in ascending order", async () => {
      const response = await loggedAxiosGet(
        `${baseUrl}/dotnetregistries/nuget.org/packages?filter=name=*Json*&limit=6&sort=packageid=asc`
      );
      ...
      console.log(
        `Packages sorted by packageid ascending: ${packages
          .slice(0, 3)
          .join(", ")}...`
      );
    });
    ...
    it("should handle sort parameter with no results", async () => {
      const response = await loggedAxiosGet(
        `${baseUrl}/dotnetregistries/nuget.org/packages?filter=name=*NonExistentPackage12345*&sort=name=desc&limit=5`
      );
      ...
      console.log(
        `Sort on empty results handled gracefully: ${packages.length} packages`
      );
    });

    it("should combine filtering and sorting effectively", async () => {
      const response = await loggedAxiosGet(
        `${baseUrl}/dotnetregistries/nuget.org/packages?filter=name=*Microsoft*&sort=name=desc&limit=7`
      );
      ...
      console.log(
        `Combined filter + sort works: ${packages.slice(0, 3).join(", ")}...`
      );
    });
  });
  ...
  describe("Inline Functionality Tests", () => {
    ...
    it("should handle inline flag in group endpoint", async () => {
      const response = await loggedAxiosGet(
        `${baseUrl}/dotnetregistries/nuget.org?inline=true`
      );
    ...
    it("should handle inline flag in packages collection", async () => {
      const response = await loggedAxiosGet(
        `${baseUrl}/dotnetregistries/nuget.org/packages?inline=true&limit=3`
      );
      ...
      console.log(
        `Successfully handled inline flag at packages collection: ${packages.length} packages`
      );
    });
  ...
  describe("Combined Filter, Sort, and Inline Tests", () => {
    it("should handle filter + sort + inline together", async () => {
      const response = await loggedAxiosGet(
        `${baseUrl}/dotnetregistries/nuget.org/packages?filter=name=*Microsoft*&sort=name=desc&inline=true&limit=5`
      );
      ...
      console.log(
        `Combined filter + sort + inline works: ${packages
          .slice(0, 3)
          .join(", ")}...`
      );
    });
    ...
    it("should handle complex filtering with sorting on unfiltered results", async () => {
      ...
      const cachedResponse = await loggedAxiosGet(
        `${baseUrl}/dotnetregistries/nuget.org/packages?sort=name=asc&limit=5`
      );
      ...
      if (cachedPackages.length > 1) {
        const sortedCachedPackages = [...cachedPackages].sort((a, b) =>
          a.toLowerCase().localeCompare(b.toLowerCase())
        );
      ...
      console.log(
        `Sorting on cached results works: ${cachedPackages
          .slice(0, 3)
          .join(", ")}...`
      );
    });
    ...
    it("should handle edge case with zero results and all flags", async () => {
      const response = await loggedAxiosGet(
        `${baseUrl}/dotnetregistries/nuget.org/packages?filter=name=*ThisDefinitelyDoesNotExist99999*&sort=name=desc&inline=true&limit=10`
      );
      ...
      console.log(
        `Edge case with no results handled gracefully: ${packages.length} packages`
      );
    });
  });
  ...
  describe("Pagination with Filter and Sort", () => {
    it("should include proper pagination headers with filtering", async () => {
      const response = await loggedAxiosGet(
        `${baseUrl}/dotnetregistries/nuget.org/packages?filter=name=*Microsoft*&limit=5&offset=0`
      );
    ...
    it("should maintain filter and sort parameters in pagination links", async () => {
      const response = await loggedAxiosGet(
        `${baseUrl}/dotnetregistries/nuget.org/packages?filter=name=*System*&sort=name=desc&limit=3&offset=0`
      );
      ...
      if (response.headers.link) {
        const linkHeader = response.headers.link;
        expect(linkHeader).to.include("filter=name%3D*System*");
        expect(linkHeader).to.include("sort=name%3Ddesc");
      }

      console.log(
        "Pagination links correctly maintain filter and sort parameters"
      );
    });
  });
  ...
  describe("Response Headers and Compliance", () => {
    it("should include proper xRegistry headers with all flags", async () => {
      const response = await loggedAxiosGet(
        `${baseUrl}/dotnetregistries/nuget.org/packages?filter=name=*Json*&sort=name=asc&inline=true&limit=3`
      );
      ...
      console.log(
        "All required xRegistry headers present with filter/sort/inline flags"
      );
    });
    ...
    it("should handle concurrent requests with different flag combinations", async () => {
      ...
      const requests = [
        loggedAxiosGet(
          `${baseUrl}/dotnetregistries/nuget.org/packages?filter=name=*Microsoft*&limit=3`
        ),
        loggedAxiosGet(
          `${baseUrl}/dotnetregistries/nuget.org/packages?filter=name=*System*&sort=name=asc&inline=true&limit=3`
        ),
        loggedAxiosGet(
          `${baseUrl}/dotnetregistries/nuget.org/packages?inline=true&limit=3`
        ),
      ...
      responses.forEach((response, index) => {
        ...
        expect(response.data).to.be.an("object");
        console.log(
          `Concurrent request ${index + 1} succeeded with ${
      ...
      console.log(
        "All concurrent requests with different flag combinations succeeded"
      );
    });
  });
});
