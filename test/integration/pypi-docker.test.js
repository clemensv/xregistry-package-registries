...
describe('PyPI Docker Integration Tests', function() {
  ...
  before(async function() {
    ...
    containerName = `pypi-test-${Date.now()}-${Math.floor(Math.random() * 1000)}`;
    ...
    await executeCommand(
      ...
      `-e XREGISTRY_PYPI_PORT=3000 ` +
      `-e XREGISTRY_PYPI_QUIET=false ` +
    ...
    it('should respond to a specific package (requests)', async () => {
      try {
        ...
        if (error.response && error.response.status === 404) {
          console.log('requests package not found - this may be expected if the external registry is unavailable');
  ...
  describe('Error Handling', () => {
    it('should return 404 for non-existent registry', async () => {
      try {
        await loggedAxiosGet(`${baseUrl}/pythonregistries/non-existent-registry`);
