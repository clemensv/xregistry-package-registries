...
describe("OCI Docker Integration Tests", function () {
  ...
  before(async function () {
    ...
    containerName = `oci-test-${Date.now()}-${Math.floor(
      Math.random() * 1000
    )}`;
    ...
    it("should respond to a specific OCI registry (microsoft)", async () => {
      try {
        const response = await loggedAxiosGet(
          `${baseUrl}/containerregistries/microsoft`
        );
        expect(response.status).to.equal(200);
        expect(response.data).to.be.an("object");
        expect(response.data).to.have.property("name", "microsoft");
      } catch (error) {
        // Microsoft registry may not be configured in test environment
        if (error.response && error.response.status === 404) {
          console.log(
            "Microsoft registry not found - may be expected if not configured in backends"
          );
  ...
  describe("Package Endpoints", () => {
    it("should respond to images endpoint for microsoft", async () => {
      const response = await loggedAxiosGet(
        `${baseUrl}/containerregistries/microsoft/images`
      );
      expect(response.status).to.equal(200);
      expect(response.data).to.be.an("object");
    });
    ...
    it("should respond to a specific image (dotnet/runtime)", async () => {
      try {
        const response = await loggedAxiosGet(
          `${baseUrl}/containerregistries/microsoft/images/dotnet~runtime`
        );
        expect(response.status).to.equal(200);
        expect(response.data).to.be.an("object");
        ...
        if (error.response && error.response.status === 404) {
          console.log(
            "dotnet/runtime image not found - this may be expected if the external registry is unavailable"
          );
          expect(error.response.status).to.equal(404);
        } else {
          throw error;
  ...
  describe("Error Handling", () => {
    it("should return 404 for non-existent registry", async () => {
      try {
        await loggedAxiosGet(
          `${baseUrl}/containerregistries/non-existent-registry`
        );
    ...
    it("should return 404 for non-existent image", async () => {
      try {
        await loggedAxiosGet(
          `${baseUrl}/containerregistries/microsoft/images/non-existent-image-123456789`
        );
        expect.fail("Should have thrown an error");
      } catch (error) {
        if (error.response) {
