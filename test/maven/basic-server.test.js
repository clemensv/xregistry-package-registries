...
describe("Maven Basic Server Functionality", function () {
  ...
  describe("Core Endpoints", function () {
    it("should return registry root with correct structure", async function () {
      ...
      expect(response.data).to.have.property("javaregistriesurl");
  ...
  describe("Package Operations", function () {
    ...
    it("should return packages for a registry", async function () {
      ...
      const response = await axios.get(
        `${baseUrl}/javaregistries/${firstRegistryName}/packages`
      );
    ...
    it("should support pagination for packages", async function () {
      ...
      const response = await axios.get(
        `${baseUrl}/javaregistries/${firstRegistryName}/packages?limit=1`
      );
    ...
    it("should support group IDs for Maven artifacts", async function () {
      ...
      try {
        const response = await axios.get(
          `${baseUrl}/javaregistries/${firstRegistryName}/packages/org.apache.commons:commons-lang3`
        );
        expect(response.status).to.equal(200);
      } catch (error) {
    ...
    it("should return packages for a group", async function () {
      const response = await axios.get(
        `${baseUrl}/javaregistries/central.maven.org/packages`
      );
    ...
    it("should support xRegistry filter operators", async function () {
      const operators = [
        "name=*jackson*", // Wildcard - Jackson is common in Maven Central
        "name!=*nonexistent*", // Not equals with wildcard
        "name=junit:junit", // Exact match (if junit exists)
      ];

      for (const filter of operators) {
        const response = await axios.get(
          `${baseUrl}/javaregistries/central.maven.org/packages?filter=${encodeURIComponent(
            filter
          )}&limit=3`
        );
    ...
    it("should handle empty filter results gracefully", async function () {
      const response = await axios.get(
        `${baseUrl}/javaregistries/central.maven.org/packages?filter=name=*ThisPackageDefinitelyDoesNotExist12345*&limit=5`
      );
    ...
    it("should filter for common Maven packages", async function () {
      const testCases = [
        { filter: "name=*spring*", name: "Spring Framework packages" },
        { filter: "name=*jackson*", name: "Jackson JSON packages" },
        { filter: "name=*apache*", name: "Apache packages" },
      ];
  ...
  async function startServer(port) {
    ...
    return new Promise((resolve, reject) => {
      ...
      childProcess.stdout.on("data", (data) => {
        ...
        if (
          stdout.includes("Service started") ||
          stdout.includes(
            `Maven Central xRegistry server started on port ${port}`
          )
        ) {
          if (!started) {
            started = true;
      ...
      childProcess.stderr.on("data", (data) => {
        ...
        if (
          stderr.includes("Service started") ||
          stderr.includes(
            `Maven Central xRegistry server started on port ${port}`
          )
        ) {
