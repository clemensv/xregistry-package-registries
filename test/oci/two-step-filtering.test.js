...
/**
 * Two-Step Filtering Tests for OCI Registry
...
const SERVER_URL = process.env.OCI_SERVER_URL || 'http://localhost:3500';
const REQUEST_TIMEOUT = 30000;

describe('OCI Two-Step Filtering', function() {
  this.timeout(60000);
  ...
  before(async function() {
    try {
      ...
      console.log('✅ OCI server is available for testing');
    } catch (error) {
      console.log('⚠️ OCI server not available at ${SERVER_URL}');
      this.skip();
    }
  });
  ...
  // Add more OCI-specific tests here
});
