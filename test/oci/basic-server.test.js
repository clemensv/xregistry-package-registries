...
describe('OCI Basic Server Functionality', function() {
  ...
  describe('Core Endpoints', function() {
    it('should return registry root with correct structure', async function() {
      ...
      expect(response.data).to.have.property('capabilitiesurl');      expect(response.data).to.have.property('containerregistriesurl');
    ...
    it('should return containerregistries collection', async function() {
      ...
      console.log('containerregistries response:', JSON.stringify(response.data, null, 2));
      expect(response.status).to.equal(200);
      ...
      const config = JSON.parse(fs.readFileSync(path.join(__dirname, '../../oci/config.json'), 'utf8'));
    ...
    it('should handle 404 for nonexistent registry', async function() {
      try {
        await axios.get(`${baseUrl}/containerregistries/nonexistent-registry-xyz`);
    ...
    before(async function() {
      ...
      const response = await axios.get(`${baseUrl}/containerregistries/${firstRegistryName}/images`);
      ...
      const supportsPagination = capabilitiesResponse.data.capabilities && capabilitiesResponse.data.capabilities.pagination === true;
      ...
      const limit = 1;
      const response = await axios.get(`${baseUrl}/containerregistries/${firstRegistryName}/images?limit=${limit}`);
      expect(response.status).to.equal(200);
      ...
      if (Object.keys(response.data).length === limit) {
        ...
        expect(linkHeader, 'Link header with rel="next" must be present if more records exist').to.match(/<[^>]+>;\s*rel="next"/);
      } else {
  ...
  async function startServer(port, configPath) {
    ...
    return new Promise((resolve, reject) => {
      const childProcess = spawn('node', [serverPath, '--port', port, '--config-file', configPath], {
        shell: false,
      ...
      childProcess.stdout.on('data', (data) => {
        ...
        if (stdout.includes('Server listening on port') || stdout.includes(`listening on port ${port}`)) {
          if (!started) {
            started = true;
