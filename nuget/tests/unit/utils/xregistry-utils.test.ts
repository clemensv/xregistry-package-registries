...
const createMockRequest = (query: Record<string, any> = {}) => ({
    query,
    protocol: 'http',
    get: (header: string) => header === 'host' ? 'localhost:3300' : undefined,
    path: '/test',
}) as any;
...
describe('xRegistry Utilities', () => {
    describe('generateXRegistryEntity', () => {
        test('should generate valid xRegistry entity with required fields', () => {
            const options = {
                ...
                parentUrl: '/dotnetregistries/nuget.org/packages',
                type: 'package',
            };
            ...
            expect(entity.xid).toBe('/dotnetregistries/nuget.org/packages/test-package');
        ...
        test('should generate proper self URL', () => {
            ...
            const entity = generateXRegistryEntity(options);
            expect(entity.self).toMatch(/^http:\/\/localhost:3300\/dotnetregistries\/nuget\.org\/packages\/test-package$/);
        });
    });
    ...
    describe('xRegistry Compliance', () => {
        ...
        test('should handle scoped package names in xids', () => {
            ...
            const entity = generateXRegistryEntity(options);
            expect(entity.xid).toBe('/dotnetregistries/nuget.org/packages/%40types%2Fnode');
