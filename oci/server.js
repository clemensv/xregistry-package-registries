...
const argv = yargs
  ...
  .option('cache-dir', {
    ...
    .option('oci-backends', {
        type: 'string',
    describe: 'JSON string of OCI backend configurations. Overrides config-file if set.',
        coerce: (arg) => {
      if (arg === undefined) return undefined;
            try {
                const parsed = JSON.parse(arg);
                if (!Array.isArray(parsed)) throw new Error('OCI backends must be an array.');
                parsed.forEach(backend => {
                    if (!backend.name || !backend.registryUrl) {
                        throw new Error('Each OCI backend must have a name and registryUrl.');
                    }
  ...
  if (API_KEY) {
    ...
    if (!authHeader || !authHeader.startsWith('Bearer ') || authHeader.substring(7) !== API_KEY) {
      return res.status(401).json(
        createErrorResponse("unauthorized", "Unauthorized", 401, req.originalUrl, "Invalid or missing API key")
      );
    }
  }
  next();
...
app.use((req, res, next) => {
  if (req.path.endsWith('$details')) {
    ...
    req.url = basePath + (req.url.includes('?') ? '&' : '?') + Object.entries(req.query).map(([k, v]) => `${k}=${v}`).join('&');
    req.path = basePath;
...
function setXRegistryHeaders(res, data) {
  ...
  res.set('Access-Control-Allow-Headers', 'Content-Type, Authorization, X-Registry-*');
...
function validateAgainstSchema(data, entityType) {
  ...
  try {
    ...
    throw createErrorResponse("invalid_data", "Schema validation failed", 400, 
      "Schema validation", error.message, data);
  }
}
...
async function cachedGet(url, headers = {}) {
  try {
    ...
    if (error.response) {
      const newError = new Error(`HTTP ${error.response.status}: ${error.response.statusText}`);
...
function makeAllUrlsAbsolute(req, obj) {
  ...
  function processValue(value, key) {
    if (typeof value === 'string') {
      if (typeof key === 'string' && (key === 'self' || key === 'docs' || key.endsWith('url'))) {
...
function handleCollectionsFlag(req, data) {
  if (req.query.collections === 'false') {
    ...
    Object.keys(result).forEach(key => {
      if (typeof key === 'string' && key.endsWith('url') && !key.startsWith('self')) {
        delete result[key];
      }
    });
    return result;
...
async function getCachePath(backendName, imageName, version) {
  ...
  const safeImage = sanitize(imageName.replace(/\//g, '_'));
    ...
    const cachePath = path.join(CACHE_DIR, safeBackend, safeImage, `${safeVersion}.json`);
...
async function readFromCache(backendName, imageName, version) {
    ...
    if (!QUIET_MODE) {
      console.log(`Cache hit for ${backendName}/${imageName}/${version || '_all_versions_'}`);
    }
        return JSON.parse(data);
    } catch (error) {
        if (error.code !== 'ENOENT') {
      console.warn(`Error reading from cache ${backendName}/${imageName}/${version || '_all_versions_'}: ${error.message}`);
        }
        return null;
    }
}
...
async function writeToCache(backendName, imageName, version, data) {
    ...
    if (!QUIET_MODE) {
      console.log(`Wrote to cache ${backendName}/${imageName}/${version || '_all_versions_'}`);
    }
    } catch (error) {
    console.warn(`Error writing to cache ${backendName}/${imageName}/${version || '_all_versions_'}: ${error.message}`);
    }
}
...
async function getAuthToken(backend, scope) {
    ...
    try {
      if (!QUIET_MODE) {
        console.log(`[Auth] Backend ${backend.name}: Attempting anonymous authentication flow.`);
      }
      
      const wwwAuthenticateResponse = await axios.get(`${backend.registryUrl}/v2/`, {
        validateStatus: () => true // Don't throw on 401
      });

            const authHeader = wwwAuthenticateResponse.headers['www-authenticate'];
            if (authHeader && authHeader.startsWith('Bearer ')) {
                const params = authHeader.substring('Bearer '.length).split(',').reduce((acc, part) => {
                ...
                if (params.realm && params.service) {
                    let authUrl = `${params.realm}?service=${encodeURIComponent(params.service)}`;
                    if (scope) {
                        authUrl += `&scope=${encodeURIComponent(scope)}`;
                    }
          
          const tokenResponse = await axios.get(authUrl, { validateStatus: () => true });
          
          if (tokenResponse.status >= 200 && tokenResponse.status < 300 && 
              (tokenResponse.data.token || tokenResponse.data.access_token)) {
            if (!QUIET_MODE) {
              console.log(`[Auth] Backend ${backend.name}: Successfully acquired anonymous token.`);
            }
      ...
      console.warn(`[Auth] Backend ${backend.name}: Anonymous token acquisition attempt failed: ${err.message}`);
      return null;
    }
  }
...
async function ociRequest(backend, requestPath, method = 'GET', additionalHeaders = {}, req = null) {
  ...
  try {
    ...
    if (token) {
      ...
      headers['Authorization'] = `Basic ${Buffer.from(`${backend.username}:${backend.password}`).toString('base64')}`;
    }
...
function extractComprehensiveMetadata(imageConfig, defaultDescription) {
  ...
  if (metadata.description === defaultDescription && labels['org.opencontainers.image.title']) {
...
function extractPlatformInfo(manifest) {
  ...
  if (manifest.mediaType && (manifest.mediaType.includes('manifest.list') || manifest.mediaType.includes('index'))) {
...
function extractBuildHistory(imageConfig) {
  ...
  return imageConfig.history.map((entry, index) => ({
    step: index + 1,
...
app.get('/', (req, res) => {
  ...
  let rootResponse = {
    ...
    [`${GROUP_TYPE}count`]: OCI_BACKENDS.length,
      ...
      flags: ["collections", "doc", "epoch", "filter", "inline", "limit", "offset", "nodefaultversionid", "nodefaultversionsticky", "noepoch", "noreadonly", "offered", "schema", "setdefaultversionid", "specversion"],
      mutable: ["model"],
      pagination: true,
...
app.get('/capabilities', (req, res) => {
  ...
  const response = {
    ...
    description: "This registry supports read-only operations and model discovery for OCI container registries."
  };
...
app.get(`/${GROUP_TYPE}/:groupid`, (req, res) => {
  ...
  const baseUrl = BASE_URL || `${req.protocol}://${req.get('host')}`;
    ...
    if (!backend) {
    const err = createErrorResponse('not_found', `Group (OCI Backend) '${groupid}' not found.`, 404, 
      `${baseUrl}/${GROUP_TYPE}/${groupid}`, `The OCI backend named '${groupid}' is not configured.`, groupid);
...
app.get(`/${GROUP_TYPE}/:groupid/${RESOURCE_TYPE}`, async (req, res) => {
  ...
  let totalCount = 0;
    ...
    if (!QUIET_MODE) {
      console.log(`[Catalog] Backend '${groupid}': Catalog is disabled for this backend, returning empty map.`);
    }
    } else {
        try {
      ...
      let allRepositories = [];
            ...
            while (fetchUrl) {
        if (!QUIET_MODE) {
          console.log(`[Catalog] Backend '${groupid}': Attempting to list images from ${backend.registryUrl}${fetchUrl}`);
        }
          ...
          allRepositories = allRepositories.concat(catalog.repositories);
              ...
              if (!QUIET_MODE) {
                console.log(`[Catalog] Backend '${groupid}': Found next page for _catalog: ${fetchUrl}`);
              }
                        } else {
                            fetchUrl = null; 
                        }
      ...
      // Apply pagination to the repositories
      const paginatedRepositories = allRepositories.slice(offset, offset + limit);
      
      paginatedRepositories.forEach(repoName => {
        ...
        allImages[imageIdWithTilde] = {
          ...
          self: `${baseUrl}/${GROUP_TYPE}/${groupid}/${RESOURCE_TYPE}/${encodeURIComponent(imageIdWithTilde)}`,
          versions: `${baseUrl}/${GROUP_TYPE}/${groupid}/${RESOURCE_TYPE}/${encodeURIComponent(imageIdWithTilde)}/versions`,
        ...
        allImages[imageIdWithTilde] = handleDocFlag(req, allImages[imageIdWithTilde]);
        allImages[imageIdWithTilde] = handleEpochFlag(req, allImages[imageIdWithTilde]);
        allImages[imageIdWithTilde] = handleNoReadonlyFlag(req, allImages[imageIdWithTilde]);
        allImages[imageIdWithTilde] = handleSchemaFlag(req, allImages[imageIdWithTilde], 'resource');
      });
      
      if (!QUIET_MODE) {
        console.log(`[Catalog] Backend '${groupid}': Successfully listed ${totalCount} images (showing ${Object.keys(allImages).length}).`);
      }

        } catch (error) {
            if (error.statusCode === 404 || error.statusCode === 401 || error.statusCode === 403) {
        console.warn(`[Catalog] Backend '${groupid}': Listing images (_catalog at ${baseCatalogPath}) failed (status: ${error.statusCode}) or is not supported/permitted. Detail: ${error.detail}.`);
            } else {
        console.error(`[Catalog] Backend '${groupid}': Error listing images:`, error);
            }
            allImages = {};
      totalCount = 0;
      console.warn(`[Catalog] Backend '${groupid}': Returning empty map of images due to previous error or disabled catalog.`);
    }
  }
...
app.get(`/${GROUP_TYPE}/:groupid/${RESOURCE_TYPE}/:imageid`, async (req, res) => {
  ...
  const originalImageId = decodeImageName(imageid);
    ...
    let resourceData = {
      ...
      self: `${baseUrl}/${GROUP_TYPE}/${groupid}/${RESOURCE_TYPE}/${encodeURIComponent(imageid)}`,
      versions: `${baseUrl}/${GROUP_TYPE}/${groupid}/${RESOURCE_TYPE}/${encodeURIComponent(imageid)}/versions`,
      registry: backend.registryUrl,
      repository: originalImageId,
      namespace: originalImageId.includes('/') ? originalImageId.split('/')[0] : null,
    };
    ...
    setXRegistryHeaders(res, resourceData);
      ...
      const err = createErrorResponse('not_found', `Resource (Image) '${imageid}' not found in '${groupid}'.`, 404,
        `${baseUrl}/${GROUP_TYPE}/${groupid}/${RESOURCE_TYPE}/${encodeURIComponent(imageid)}`, error.detail || `Image ${originalImageId} not found.`, { groupid, imageid });
      return res.status(404).json(err);
    }
    console.error(`Error fetching image details for ${groupid}/${originalImageId}:`, error);
    const err = createErrorResponse('server_error', 'Failed to retrieve image details.', 500,
      `${baseUrl}/${GROUP_TYPE}/${groupid}/${RESOURCE_TYPE}/${encodeURIComponent(imageid)}`, error.message, null);
    return res.status(500).json(err);
  }
});

// Meta endpoint
app.get(`/${GROUP_TYPE}/:groupid/${RESOURCE_TYPE}/:imageid/meta`, async (req, res) => {
  let { groupid, imageid } = req.params;
  imageid = decodeURIComponent(imageid);
  const backend = OCI_BACKENDS.find(b => b.name === groupid);
  const baseUrl = BASE_URL || `${req.protocol}://${req.get('host')}`;
  ...
  const originalImageId = decodeImageName(imageid);
  const resourceBasePath = `${baseUrl}/${GROUP_TYPE}/${groupid}/${RESOURCE_TYPE}/${encodeURIComponent(imageid)}`;
  const metaUrl = `${resourceBasePath}/meta`;

  try {
    const tagsResponse = await ociRequest(backend, `/v2/${originalImageId}/tags/list`);
    const tags = tagsResponse.body.tags || [];
    
    // Try to determine the latest tag (prefer 'latest', then most recent)
    let latestTag = tags.includes('latest') ? 'latest' : (tags.length > 0 ? tags[0] : null);
    const defaultVersionUrl = latestTag ? `${resourceBasePath}/versions/${encodeURIComponent(latestTag)}` : null;
    ...
    const err = createErrorResponse("not_found", "Image not found", 404, metaUrl, 
      `The image '${originalImageId}' could not be found`, imageid);
    res.status(404).json(err);
  }
});

// Doc endpoint
app.get(`/${GROUP_TYPE}/:groupid/${RESOURCE_TYPE}/:imageid/doc`, async (req, res) => {
  let { groupid, imageid } = req.params;
  imageid = decodeURIComponent(imageid);
  const backend = OCI_BACKENDS.find(b => b.name === groupid);
  const baseUrl = BASE_URL || `${req.protocol}://${req.get('host')}`;
  ...
  try {
    ...
    const docResponse = {
      ...
      self: `${baseUrl}/${GROUP_TYPE}/${groupid}/${RESOURCE_TYPE}/${encodeURIComponent(imageid)}/doc`,
    };
    ...
    const err = createErrorResponse("not_found", "Image documentation not found", 404, 
      `${baseUrl}/${GROUP_TYPE}/${groupid}/${RESOURCE_TYPE}/${encodeURIComponent(imageid)}/doc`,
      `Documentation for image '${originalImageId}' could not be found`, imageid);
    res.status(404).json(err);
    }
});

// Versions (Tags)
app.get(`/${GROUP_TYPE}/:groupid/${RESOURCE_TYPE}/:imageid/versions`, async (req, res) => {
    let { groupid, imageid } = req.params;
    imageid = decodeURIComponent(imageid);
    const backend = OCI_BACKENDS.find(b => b.name === groupid);
  const baseUrl = BASE_URL || `${req.protocol}://${req.get('host')}`;
  ...
  if (limit <= 0) {
    ...
    const cachedData = await readFromCache(groupid, originalImageId, cacheKeyVersion);
    if (cachedData) {
    ...
    setXRegistryHeaders(res, { epoch: 1 });
    return res.json(req.query.collections === 'true' ? { collections: paginatedVersions } : paginatedVersions);
    }

    try {
        const ociResponse = await ociRequest(backend, `/v2/${originalImageId}/tags/list`);
    ...
    for (const tag of paginatedTags) {
      ...
      let imageConfig = null;
            try {
                const manifestResponse = await ociRequest(backend, `/v2/${originalImageId}/manifests/${tag}`);
        ...
        console.log(`[DEBUG] Processing manifest for ${originalImageId}:${tag}`);
        console.log(`[DEBUG] Manifest mediaType: ${manifest.mediaType}`);
        console.log(`[DEBUG] Manifest schemaVersion: ${manifest.schemaVersion}`);
        console.log(`[DEBUG] Manifest has manifests array: ${!!manifest.manifests}`);
        ...
        if (manifest.schemaVersion === 1 && manifest.history && manifest.history.length > 0) {
          ...
          if (v1CompatString) {
            try {
              ...
              console.warn(`Failed to parse v1Compatibility for ${originalImageId}:${tag}: ${e.message}`);
            }
          }
        } else if (manifest.mediaType && (manifest.mediaType.includes('manifest.list') || manifest.mediaType.includes('index'))) {
          // Handle manifest lists (multi-platform images)
          console.log(`[MANIFEST_LIST_DEBUG] Processing manifest list for ${originalImageId}:${tag}`);
          console.log(`[MANIFEST_LIST_DEBUG] Manifest mediaType: ${manifest.mediaType}`);
          console.log(`[MANIFEST_LIST_DEBUG] Manifest has manifests array: ${!!manifest.manifests}`);
          console.log(`[MANIFEST_LIST_DEBUG] Manifests count: ${manifest.manifests ? manifest.manifests.length : 0}`);
          
          if (manifest.manifests && manifest.manifests.length > 0) {
            console.log(`[MANIFEST_LIST_DEBUG] Available platforms:`);
            manifest.manifests.forEach((m, i) => {
              console.log(`[MANIFEST_LIST_DEBUG]   Platform ${i}: ${m.platform?.os}/${m.platform?.architecture} - digest: ${m.digest?.substring(0, 16)}...`);
            });
            ...
            const linuxAmd64 = manifest.manifests.find(m => 
              m.platform && m.platform.architecture === 'amd64' && m.platform.os === 'linux'
            );
            if (linuxAmd64) {
              console.log(`[MANIFEST_LIST_DEBUG] Found linux/amd64 platform, using it`);
              platformManifest = linuxAmd64;
            } else {
              console.log(`[MANIFEST_LIST_DEBUG] No linux/amd64 found, using first platform: ${platformManifest.platform?.os}/${platformManifest.platform?.architecture}`);
            }
            
            console.log(`[MANIFEST_LIST_DEBUG] Selected platform: ${platformManifest.platform?.os}/${platformManifest.platform?.architecture}`);
            console.log(`[MANIFEST_LIST_DEBUG] Platform digest: ${platformManifest.digest}`);
            
            try {
              console.log(`[MANIFEST_LIST_DEBUG] Fetching platform-specific manifest: /v2/${originalImageId}/manifests/${platformManifest.digest}`);
              
              // Fetch the platform-specific manifest
              const platformResponse = await ociRequest(backend, `/v2/${originalImageId}/manifests/${platformManifest.digest}`);
              const platformManifestData = platformResponse.body;
              
              console.log(`[MANIFEST_LIST_DEBUG] Platform manifest fetched successfully`);
              console.log(`[MANIFEST_LIST_DEBUG] Platform manifest mediaType: ${platformManifestData.mediaType}`);
              console.log(`[MANIFEST_LIST_DEBUG] Platform manifest has config: ${!!platformManifestData.config}`);
              console.log(`[MANIFEST_LIST_DEBUG] Platform manifest has layers: ${!!platformManifestData.layers}, count: ${platformManifestData.layers?.length || 0}`);
              ...
              console.log(`[MANIFEST_LIST_DEBUG] Extracted from platform: arch=${architecture}, os=${os}, size=${size}`);
              
              // If platform manifest has config, fetch that too for more details
              if (platformManifestData.config && platformManifestData.config.digest) {
                console.log(`[MANIFEST_LIST_DEBUG] Fetching image config: /v2/${originalImageId}/blobs/${platformManifestData.config.digest}`);
                ...
                try {
                  ...
                  console.log(`[MANIFEST_LIST_DEBUG] Config architecture: ${imageConfig.architecture}`);
                  console.log(`[MANIFEST_LIST_DEBUG] Config os: ${imageConfig.os}`);
                  console.log(`[MANIFEST_LIST_DEBUG] Config has labels: ${!!imageConfig.config?.Labels}`);
                  ...
                  if (imageConfig.architecture) {
                    ...
                    console.log(`[MANIFEST_LIST_DEBUG] Updated architecture from config: ${architecture}`);
                  }
                  if (imageConfig.os) {
                    os = imageConfig.os;
                    console.log(`[MANIFEST_LIST_DEBUG] Updated OS from config: ${os}`);
                  }
                  
                  // Extract description from image labels  
                  description = extractImageDescription(imageConfig, description);
                  console.log(`[MANIFEST_LIST_DEBUG] Extracted description: ${description.substring(0, 50)}...`);
                  
                  // Calculate size from layers if not available
                  if (!size && platformManifestData.layers && platformManifestData.layers.every(l => typeof l.size === 'number')) {
                    size = platformManifestData.layers.reduce((acc, layer) => acc + layer.size, 0);
                    console.log(`[MANIFEST_LIST_DEBUG] Calculated size from layers: ${size} bytes`);
                  }
                } catch (configError) {
              ...
              if (platformManifestData.layers) {
                layers = platformManifestData.layers.map(l => ({ digest: l.digest, size: l.size, mediaType: l.mediaType }));
                console.log(`[MANIFEST_LIST_DEBUG] Set layers from platform manifest: ${layers.length} layers`);
              }
              
              console.log(`[MANIFEST_LIST_DEBUG] Final values: arch=${architecture}, os=${os}, size=${size}, layers=${layers.length}`);
              
            } catch (platformError) {
              console.error(`[MANIFEST_LIST_DEBUG] Error fetching platform manifest: ${platformError.message}`);
              console.error(`[MANIFEST_LIST_DEBUG] Platform error status: ${platformError.statusCode}`);
              console.error(`[MANIFEST_LIST_DEBUG] Platform error detail: ${platformError.detail}`);
              console.warn(`Could not fetch platform manifest for ${groupid}/${originalImageId}:${tag}: ${platformError.message}`);
            }
          } else {
            console.log(`[MANIFEST_LIST_DEBUG] No manifests array found or empty`);
          }
        } else if ((manifest.schemaVersion === 2 && manifest.mediaType && manifest.mediaType.includes('manifest') && !manifest.mediaType.includes('manifest.list')) || 
                   (manifest.mediaType && manifest.mediaType.includes('oci.image.manifest'))) {
          ...
          if (manifest.config && manifest.config.digest) {
            try {
              const configResponse = await ociRequest(backend, `/v2/${originalImageId}/blobs/${manifest.config.digest}`);
              imageConfig = configResponse.body;
              createdAt = imageConfig.created || createdAt;
              ...
              if (manifest.layers && manifest.layers.every(l => typeof l.size === 'number')) {
                size = manifest.layers.reduce((acc, layer) => acc + layer.size, 0);
              }
            } catch (configError) {
              console.warn(`Could not fetch config blob for ${groupid}/${originalImageId}:${tag}: ${configError.message}`);
            }
          }
        } else if (manifest.mediaType && (manifest.mediaType.includes('manifest.list') || manifest.mediaType.includes('index'))) {
          // Handle manifest lists (multi-platform images)
        ...
        console.warn(`Could not fetch manifest for ${groupid}/${originalImageId}:${tag} to get created date: ${manifestError.message}`);
            }
            ...
            if (imageConfig) {
              comprehensiveMetadata = extractComprehensiveMetadata(imageConfig, description);
        ...
        versionid: tag,
        self: `${baseUrl}/${GROUP_TYPE}/${groupid}/${RESOURCE_TYPE}/${encodeURIComponent(imageid)}/versions/${encodeURIComponent(tag)}`,
                createdat: createdAt,
        ...
        metadata: {
          ...
          ...(comprehensiveMetadata?.labels && Object.keys(comprehensiveMetadata.labels).length > 0 && {
            oci_labels: comprehensiveMetadata.labels
          }),
          // Add configuration details
          ...(comprehensiveMetadata?.environment && { environment: comprehensiveMetadata.environment }),
          ...(comprehensiveMetadata?.workingDir && { working_dir: comprehensiveMetadata.workingDir }),
          ...(comprehensiveMetadata?.entrypoint && { entrypoint: comprehensiveMetadata.entrypoint }),
          ...(comprehensiveMetadata?.cmd && { cmd: comprehensiveMetadata.cmd }),
          ...(comprehensiveMetadata?.user && { user: comprehensiveMetadata.user }),
          ...(comprehensiveMetadata?.exposedPorts && { exposed_ports: comprehensiveMetadata.exposedPorts }),
          ...(comprehensiveMetadata?.volumes && { volumes: comprehensiveMetadata.volumes })
        },
                urls: {
    ...
    console.error(`Error fetching versions for ${groupid}/${originalImageId}:`, error);
    const err = createErrorResponse(
      ...
      `${baseUrl}/${GROUP_TYPE}/${groupid}/${RESOURCE_TYPE}/${encodeURIComponent(imageid)}/versions`, 
...
app.get(`/${GROUP_TYPE}/:groupid/${RESOURCE_TYPE}/:imageid/versions/:versionid`, async (req, res) => {
  ...
  const originalImageId = decodeImageName(imageid);
    ...
    try {
        ...
        const manifestResponse = await ociRequest(backend, `/v2/${originalImageId}/manifests/${versionid}`);
        manifest = manifestResponse.body;
        digest = manifestResponse.headers['docker-content-digest'] || manifestResponse.headers['etag'] || null;
    ...
    let description = `Container image tag ${versionid}`;
        ...
        console.log(`[DEBUG] Processing manifest for ${originalImageId}:${versionid}`);
        console.log(`[DEBUG] Manifest mediaType: ${manifest.mediaType}`);
        console.log(`[DEBUG] Manifest schemaVersion: ${manifest.schemaVersion}`);
        console.log(`[DEBUG] Manifest has manifests array: ${!!manifest.manifests}`);
        console.log(`[DEBUG] Manifest has history: ${!!manifest.history}`);
        ...
        if (manifest.schemaVersion === 1 && manifest.history && manifest.history.length > 0) {
          ...
          console.warn(`Failed to parse v1Compatibility for ${originalImageId}:${versionid}: ${e.message}`);
                }
            }
            layers = manifest.fsLayers ? manifest.fsLayers.map(l => ({ digest: l.blobSum, size: null })) : [];
    } else if (manifest.mediaType && (manifest.mediaType.includes('manifest.list') || manifest.mediaType.includes('index'))) {
      // Handle manifest lists (multi-platform images) - MOVED TO SECOND POSITION
      console.log(`[MANIFEST_LIST_DEBUG] Processing manifest list for ${originalImageId}:${versionid}`);
      console.log(`[MANIFEST_LIST_DEBUG] Manifest mediaType: ${manifest.mediaType}`);
      console.log(`[MANIFEST_LIST_DEBUG] Manifest has manifests array: ${!!manifest.manifests}`);
      console.log(`[MANIFEST_LIST_DEBUG] Manifests count: ${manifest.manifests ? manifest.manifests.length : 0}`);
      ...
      if (manifest.manifests && manifest.manifests.length > 0) {
        ...
        try {
          ...
          console.error(`[MANIFEST_LIST_DEBUG] Platform error status: ${platformError.statusCode}`);
          console.error(`[MANIFEST_LIST_DEBUG] Platform error detail: ${platformError.detail}`);
          console.warn(`Could not fetch platform manifest for ${groupid}/${originalImageId}:${versionid}: ${platformError.message}`);
        }
      } else {
        console.log(`[MANIFEST_LIST_DEBUG] No manifests array found or empty`);
      }
          ...
          console.warn(`Could not fetch config blob for ${groupid}/${originalImageId}:${versionid}: ${configError.message}`);
                }
            }
            layers = manifest.layers ? manifest.layers.map(l => ({ digest: l.digest, size: l.size, mediaType: l.mediaType })) : [];
    }
    ...
    let versionData = {
      ...
      versionid: versionid,
      self: `${baseUrl}/${GROUP_TYPE}/${groupid}/${RESOURCE_TYPE}/${encodeURIComponent(imageid)}/versions/${encodeURIComponent(versionid)}`,
            createdat: createdAt,
      modifiedat: createdAt,
      // Mirror resource information structure
      registry: backend.registryUrl,
    ...
    console.error(`Error fetching version ${versionid} for ${groupid}/${originalImageId}:`, error);
    const err = createErrorResponse(
            error.statusCode === 404 ? 'not_found' : 'server_error',
            `Failed to retrieve version '${versionid}' for image '${imageid}'.`,
      error.statusCode || 500,
      `${baseUrl}/${GROUP_TYPE}/${groupid}/${RESOURCE_TYPE}/${encodeURIComponent(imageid)}/versions/${encodeURIComponent(versionid)}`,
            error.detail || error.message, 
      error.data);
    return res.status(err.code).json(err);
  }
...
app.use((req, res, next) => {
  const err = createErrorResponse('not_found', `The requested URL was not found on this server.`, 404,
    req.originalUrl, `Endpoint ${req.method} ${req.path} does not exist.`, req.originalUrl);
  res.status(404).json(err);
});
...
app.use((err, req, res, next) => {
  ...
  const publicError = createErrorResponse('server_error', 'An unexpected internal server error occurred.', 500,
    req.originalUrl, err.message, null);
...
async function startServer() {
    ...
    if (!QUIET_MODE) {
      ...
      console.warn('No OCI backends configured. The registry will be empty. Set XREGISTRY_OCI_BACKENDS environment variable.');
        }
...
async function loadConfiguration() {
    ...
    try {
    if (!QUIET_MODE) {
      console.log(`Attempting to load OCI backends from config file: ${CONFIG_FILE_PATH}`);
    }
    const fileContent = await fs.promises.readFile(CONFIG_FILE_PATH, 'utf8');
      ...
      if (!QUIET_MODE) {
        console.log(`Successfully loaded ${backendsFromFile.length} OCI backend(s) from ${CONFIG_FILE_PATH}`);
      }
        } else {
      console.warn(`Config file ${CONFIG_FILE_PATH} found, but ociBackends array is missing or not an array.`);
        }
    } catch (error) {
        if (error.code === 'ENOENT') {
      if (!QUIET_MODE) {
        console.log(`Config file ${CONFIG_FILE_PATH} not found. Will check environment variable.`);
      }
        } else {
      console.error(`Error reading or parsing config file ${CONFIG_FILE_PATH}: ${error.message}`);
        }
    }
  ...
  // Final validation and processing for all backends
    ...
    if (!QUIET_MODE) {
      console.log(`Final effective OCI backends count: ${OCI_BACKENDS.length}`);
        OCI_BACKENDS.forEach(backend => {
        console.log(` -> Configured OCI backend: ${backend.name} | URL: ${backend.registryUrl} | Catalog Path: ${backend.catalogPath}`);
        });
    }
    }
}
